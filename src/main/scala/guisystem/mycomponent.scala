...
abstract class MyComponent extends PrintableInterface with GetSetTypedStringValueWithDefault
{
	...
	def Fire(kind:String,value:String,index:Int=0)
	{
		if(path==null) return
		val comp=this
		MyActor.queuedExecutor ! ExecutionItem(client="MyComponent.Fire",code=new Runnable{def run{
			handler(MyEvent(kind,comp,value,index=index))
		}})
	}
...
class MyText(subtype:String="field") extends MyComponent
{
	...
	def SetText(text:String)
	{
		subtype match
		{
			...
			case "pass" => node.asInstanceOf[PasswordField].setText(text)
		}
	}
	...
	def GetText:String=
	{
		subtype match
		{
			...
			case "pass" => node.asInstanceOf[PasswordField].getText()
		}
	}
	...
	def OwnCreateNode:Node=
	{
		subtype match
		{
			...
			case "pass" => node=new PasswordField()
			case _=> node=null
		}

		if(subtype!="pass") SetText(GetInitialStringValue(GS("text","")))

		if((subtype=="field")||(subtype=="pass"))
		{
			node.asInstanceOf[TextField].setOnKeyPressed(new EventHandler[KeyEvent]
			{
				def handle(ev:KeyEvent)
				{
					...
					{						
						val text=GetText
						if(subtype!="pass") StoreValue(StringData(text))
...
class MyEditableText extends MyComponent
{
	...
	def OwnCreateNode:Node=
	{
		...
		val namelabelwidth=GD("lwidth",200.0)

		namelabel.setMinWidth(namelabelwidth)
		namelabel.setMaxWidth(namelabelwidth)

		val valuelabelwidth=GD("vwidth",300.0)

		valuelabel=new Label()

		valuelabel.setMinWidth(valuelabelwidth)
		valuelabel.setMaxWidth(valuelabelwidth)

		val editbutton=new Button(GS("btext","Edit"))

		editbutton.setOnAction(new EventHandler[ActionEvent]
		{
			def handle(ev:ActionEvent)
			{	
				val itr=Builder.InputTexts(
					title=s"""Enter $name""",
					prompts=List(name),
					defaults=List(GetText),
					setstagex=true,
					stagex=valuelabel.localToScreen(valuelabel.getBoundsInLocal()).getMinX(),
					setstagey=true,
					stagey=valuelabel.localToScreen(valuelabel.getBoundsInLocal()).getMinY()
				)

				if(itr.canceled) return

				val text=itr.texts(name)
	...
	def OwnReportPrintable(level:Int=0,buff:String=""):String="editabletext"
}
...
object MyComponent
{
	...
	def FromTag(tag:Tag,parent:MyComponent=null,handler:(MyEvent)=>Unit=MyComponent.default_handler):MyComponent=
	{
		...
		kind match {
			...
			case "passfield" => comp=new MyText("pass")
			case "editabletext" => comp=new MyEditableText
